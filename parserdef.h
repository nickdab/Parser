...
void Parser::ReplaceCurrWord(std::string new_word)
{
	...
	std::string new_input = "", end = "";

	int word_end = temp_index-1;
	temp_index--;
	char curr_char = Input[temp_index];

	for (int i = 0; i < word_end+1;i++)
	{
		new_input =new_input + Input[i];
	}

	for (int i = word_end+1;i<Input.size();i++)
	{
		end = end + Input[i];
	}
...
/*	std::cout << "Replacing ";
	...
	std::cout << " with " << new_word << std::endl; */

	new_input.erase(word_start,word_end-(word_start-1));

	new_input = new_input + new_word+end;
