...
void Parser::ReplaceCurrWord(std::string new_word)
{
	...
	int word_end = temp_index;

	char curr_char = Input[temp_index];

	while ( is_whitespace(curr_char)==false && temp_index >= 0)
	{
		temp_index--;
...
/*	std::cout << "Erasing ";
	for (int i = temp_index; i <= word_end; i ++)
	{
		std::cout << Input[i];
	}
	std::cout << ",Replacing with " << new_word << " \n";*/

	word_size = word_end-temp_index+1;

	Input.erase(temp_index,word_size);
	Input.insert(temp_index,new_word);

	temp_index++;

	curr_char = Input[temp_index];

	while (is_whitespace(curr_char)==false && temp_index < Input.size())
	{
		temp_index++;
		curr_char = Input[temp_index];	
	}
	
	InputIndex = temp_index;

//	std::cout << "Next Word is " << ReadNextWord() << std::endl;
